...
const DetailVenue = () => {
  ...
  const HandleEdit = async () => {
    const { name, description, location, price, service_open, service_close } =
      formik.values;
    ...
    try {
      ...
      Swal.fire("Edit Venue", "Edit Venue Success", "success");
    } catch (error) {
      console.error(error);
    } finally {
      formik.resetForm();
  ...
  const HandleDelete = async () => {
    try {
      ...
      Swal.fire("Delete Venue", "Delete Venue Success", "success");
      navigate("/");
    } catch (error) {
      console.error(error);
      Swal.fire("Delete Venue", "Delete Venue Eror", "warning");
    }
  };
  ...
  return (
    <>
      <Layout chose="layout">
        ...
        <Layout chose="container">
          <Carousel id_user={user_id} image={image} />
          <div className="grid w-full grid-cols-2 gap-3 mb-10">
            ...
            <div className="pl-5 mt-3">
              <div className="flex w-full ">
                ...
                <div className="flex items-center justify-end w-1/5 gap-2 pr-5 text-xl font-bold text-yellow">
                  <span className="text-black">
                    {venue.average_rating === undefined
                      ? "0"
              ...
              <div className="flex items-center gap-3 mt-5 text-xl font-semibold text-gray-500">
                <BsFillGeoAltFill />
                {venue.location}{" "}
                <span className="text-white badge bg-oren">
                  {venue.distance || 0} Km{" "}
                </span>{" "}
                <BsFillClockFill /> {venue.service_time}
              </div>
              ...
              <div className="w-full pr-5 mt-5">
                <p className="mr-5">{venue.description}</p>
              </div>

              {user_id != idUser ? (
                <div className="w-full p-2 mt-10">
                  <button
                    className="w-full h-12 font-semibold text-white bg-primary rounded-xl"
                    id="btn-check-avail"
                ...
                <div className="flex justify-start w-full gap-2 p-5 mt-5">
                  <div className="mt-0 modal-action ">
                    ...
                    <label
                      htmlFor="modal-delete-venue"
                      className="flex items-center justify-center h-12 gap-3 font-semibold text-white bg-red-500 btn btn-ghost hover:text-black rounded-xl"
                      onClick={DeleteVenue}
                    >
                      Delete
                      <BsFillTrash3Fill />
          ...
          <div className={`${user_id != idUser ? "" : "hidden"}`}>
            <div className={`m-5`}>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="mt-3">
                  <div className="flex gap-2 text-2xl text-yellow">{stars}</div>
